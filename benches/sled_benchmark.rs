...
static PATH_PREFIX: LazyLock<&Path> = LazyLock::new(|| Path::new("data/unencrypted"));

// Generate benchmark tests
pub fn bench_insert(c: &mut Criterion) {
    // Generate a new database
    let path = PATH_PREFIX.join("bench_insert");
...
pub fn bench_select(c: &mut Criterion) {
    // Generate a new database
    let path = PATH_PREFIX.join("bench_select");
...
pub fn bench_select_tainted(c: &mut Criterion) {
    // Generate a new database
    let path = PATH_PREFIX.join("bench_select_tainted");
